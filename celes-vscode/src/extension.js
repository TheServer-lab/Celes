...
BLOCK_TAGS = {'title','author','date','section','header','line','blockquote','codeblock','image','linkimage','list','sublist','subsublist','table','item','newline','pagebreak','insertspace'}
INLINE_TAGS = {'bold','italic','bold+italic','underline','strike','super','sub','mark','code','link','button','checkmark','nestquote','empty'}
...
for i,(lineno,name,attrs,content) in enumerate(tokens):
    ...
    if name in ('list','sublist','subsublist'):
        b=attrs.get('bullet')
...
def inline_html(content, raw=False):
    ...
    while i<len(content):
        ...
        if name=='bold': res+=f'<strong>{im}</strong>'
        ...
        elif name=='super': res+=f'<sup>{im}</sup>'
        elif name=='sub': res+=f'<sub>{im}</sub>'
        elif name=='mark': res+=f'<mark>{im}</mark>'
            ...
            body=attrs.get('body',im)
            res+=f'<a href="{escape(inner)}" class="celes-button">{escape(body)}</a>'
        elif name=='checkmark':
...
while i<len(tokens):
    ...
    if name=='title': title=content or ''; i+=1
    elif name in ('author','date'): i+=1
    elif name=='section':
        body.append(f'<div class="celes-section"><span class="celes-section-label">{escape(content or "")}</span></div>'); i+=1
    elif name=='header':
        sz=attrs.get('size','1')
        ...
        while i<len(tokens) and tokens[i][1]=='list':
            ...
            while i<len(tokens) and tokens[i][1]=='sublist':
                ...
                sub_content=tokens[i][3] or ''; i+=1
                subsubs=[]
                while i<len(tokens) and tokens[i][1]=='subsublist':
                    ssb=tokens[i][2].get('bullet','circle')
                    st3='ol' if ssb=='number' else 'ul'
                    subsubs.append(f'<li>{inline_html(tokens[i][3] or "")}</li>'); i+=1
                ssh=f'<{st3}>{"".join(subsubs)}</{st3}>' if subsubs else ''
                subs.append(f'<li>{inline_html(sub_content)}{ssh}</li>')
...
mark{{background:#fff176;padding:.05em .2em;border-radius:2px}}
.celes-button{{display:inline-block;background:#1a73e8;color:#fff;padding:.3em .85em;border-radius:5px;text-decoration:none;font-size:.9em;font-weight:500}}
.celes-section{{display:flex;align-items:center;gap:.75rem;margin:2em 0 1em}}
.celes-section::before,.celes-section::after{{content:'';flex:1;height:1px;background:#ddd}}
.celes-section-label{{font-size:.72rem;font-weight:700;color:#999;text-transform:uppercase;letter-spacing:.08em;white-space:nowrap}}
</style></head><body>
{"".join(body)}
</body></html>"""
print(html)
