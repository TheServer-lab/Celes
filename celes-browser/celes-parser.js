// Celes 0.1.4 Parser — JavaScript port
// Parses a Celes source string and returns an HTML string.

(function (global) {

  const SELF_CLOSING = new Set(['newline', 'pagebreak', 'insertspace']);
  ...
  function inlineToHtml(content, raw = false) {
  ...
  function parseCeles(source) {
    ...
    while (i < tokens.length) {
      const [name, attrs, content] = tokens[i];

      if (name === 'comment' || name === 'declaration') { i++; continue; }

      if (name === 'title')              { title = content || ''; i++; }
      else if (name === 'author' || name === 'date') { i++; } // metadata only
        ...
        body.push(`<h${sz}>${inlineToHtml(content)}</h${sz}>`); i++;
      }
      else if (name === 'line') {
        const al = attrs.align || '';
        const st = al ? ` style="text-align:${esc(al)}"` : '';
        body.push(`<p${st}>${inlineToHtml(content)}</p>`); i++;
      }
      else if (name === 'blockquote') {
        body.push(`<blockquote>${inlineToHtml(content)}</blockquote>`); i++;
      }
      else if (name === 'codeblock') {
        body.push(`<pre><code>${esc(content || '')}</code></pre>`); i++;
      }
      else if (name === 'image') {
        body.push(`<img src="${esc(content || '')}" alt="">`); i++;
      }
      else if (name === 'linkimage') {
        const img = attrs.image || '';
        body.push(`<a href="${esc(content || '')}"><img src="${esc(img)}" alt=""></a>`); i++;
      }
      else if (name === 'table') {
        const cols = (content || '').split(',').map(c => c.trim());
        const hcells = cols.map(c => `<th>${esc(c)}</th>`).join('');
        const rows = [`<thead><tr>${hcells}</tr></thead>`];
        const brows = []; i++;
        while (i < tokens.length && tokens[i][0] === 'item') {
          const cells = (tokens[i][2] || '').split(',').map(c => c.trim());
          brows.push(`<tr>${cells.map(c => `<td>${esc(c)}</td>`).join('')}</tr>`);
          i++;
        }
        ...
        if (items.length) {
          const lt = items[0][0];
          const lis = items.map(([, c, s]) => `<li>${inlineToHtml(c)}${s}</li>`).join('');
          body.push(`<${lt}>${lis}</${lt}>`);
        }
      }
      else if (name === 'newline')     { body.push('<br>'); i++; }
      else if (name === 'pagebreak')   { body.push('<div style="page-break-after:always"></div>'); i++; }
      else if (name === 'insertspace') { body.push('<hr>'); i++; }
      else if (name === 'error')       { body.push(`<div class="celes-error">⚠ ${esc(content || '')}</div>`); i++; }
      else i++;
    }
      ...
      if (name === 'title')       { title = content || ''; i++; }
        ...
        while (i < tokens.length && tokens[i][0] === 'list') {
          ...
          while (i < tokens.length && tokens[i][0] === 'sublist') {
            const sb = tokens[i][1].bullet || 'circle';
            subType = sb === 'number' ? 'ol' : 'ul';
            subs.push(`<li>${inlineToHtml(tokens[i][2] || '')}</li>`);
            i++;
          }
          const sh = subs.length ? `<${subType}>${subs.join('')}</${subType}>` : '';
          items.push([lt, ic, sh]);
