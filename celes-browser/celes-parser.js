...
(function (global) {
  ...
  function tokenize(source) {
    ...
    while (i < lines.length) {
      let raw = lines[i];
      // Count brace depth — join continuation lines if unclosed
      let depth = 0;
      for (const ch of raw) {
        if (ch === '{') depth++;
        else if (ch === '}') depth--;
  ...
  function inlineToHtml(content, raw) {
    ...
    while (i < content.length) {
      ...
      // 0.1.5 — coloredtext
      else if (name === 'coloredtext') {
        const color = attrs.color || '';
        result += '<span style="color:' + esc(color) + '">' + im + '</span>';
      }
      else result += im;
      i = end;
    }
  ...
  function parseCeles(source) {
    ...
    while (i < tokens.length) {
      ...
      // 0.1.5 — background
        ...
        if (bgColor === null) bgColor = (content || '').trim();
        i++;
      ...
      // 0.1.5 — video
        ...
        const muted    = 'mute'     in attrs ? ' muted'    : '';
        body.push(
          '<video controls' + loop + autoplay + muted + ' class="celes-video">' +
          '<source src="' + src + '">' +
      ...
      // 0.1.5 — audio
        ...
        const autoplay = 'autoplay' in attrs ? ' autoplay' : '';
        body.push(
          '<audio controls' + loop + autoplay + ' class="celes-audio">' +
          '<source src="' + src + '">' +
    ...
    return { title: title, bgColor: bgColor, body: body.join('\n') };
  }
