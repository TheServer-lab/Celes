// Celes Renderer — content script v0.1.4

(function () {
  const url = window.location.href;

  // Detect .celes files on any protocol (file://, http://, https://)
  const isCelesFile = /\.celes(\?.*)?$/.test(url.split('#')[0]);
  if (!isCelesFile) return;

  // Robustly extract raw source text.
  // Chrome wraps local text files in: <html><head></head><body><pre>...</pre></body></html>
  // Some servers serve with content-type text/plain which also uses <pre>
  function getRawSource() {
    // Try <pre> first (Chrome's default text file viewer)
    const pre = document.querySelector('pre');
    if (pre && pre.textContent.trim().startsWith('<!Celes')) {
      return pre.textContent;
    }
    // Try body directly
    const bodyText = document.body ? document.body.innerText : '';
    if (bodyText.trim().startsWith('<!Celes')) {
      return bodyText;
    }
    // Try document text content as last resort
  ...
  function init() {
    ...
    // Safety: make sure CelesParser loaded
    ...
    const css = `
      *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .celes-button:hover { background: #1558b0 !important; }
    ...
    document.body.innerHTML = `
      <div id="celes-bar">
        <span class="badge">Celes 0.1.4</span>
  ...
  // Run immediately — document_end ensures DOM is ready
  init();
